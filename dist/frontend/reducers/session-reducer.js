...
function session() {
  ...
  switch (action.type) {
    case _actionTypeConstants.CREATE_SESSION_START:
      return createSessionStart(state);
    case _actionTypeConstants.CREATE_SESSION_ERROR:
      return createSessionError(state, action.error);
    case _actionTypeConstants.CREATE_SESSION_SUCCESS:
      return createSessionSuccess(state, action.userinfo);
    case _actionTypeConstants.VALIDATE_SESSION_START:
...
function createSessionError(state, error) {
  return state.set('state', 'SIGNIN_ERROR').set('error', error);
}

function createSessionSuccess(state, userinfo) {
  return state.set('state', 'SIGNIN_OK').set('userinfo', userinfo);
}

function validateSessionStart(state) {
  return state.set('state', 'VALIDATION_ONGOING');
}
